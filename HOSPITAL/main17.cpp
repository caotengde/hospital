...
int day_in_hospital(MedRecord* p1){//计算某一个月的天数，今年以2020年记为闰年，2月有29天
    int  i;
    int p;
    switch(i){
        case 1:p=31;break;
...
int in_hospital_day(MedRecord* p1){//用来计算所有病人实际的住院天数总和
    int p,day;
    day=0;
    while(p1->next!=NULL){
    if((int)(p1->medinf.hospital.yes==1))//判断是否住院
        {
    if((p1->medinf.hospital.intime.month)==(p1->medinf.hospital.outime.month))
        //判断住院和出院的月份是否为同一个月
    {
        int day,day1,day2;
        day1=(p1->medinf.hospital.intime.data);
        day2=(p1->medinf.hospital.outime.data);
        day=day2-day1+1+day;
        p1=p1->next;
    }else
    {

        p=day_in_hospital(p1);
        int day,day1,day2;
        day1=(p1->medinf.hospital.intime.data);
        day2=(p1->medinf.hospital.outime.data);
        day=p-day1+1+day2+day;
        p1=p1->next;
    }
    }else day=0;
    }
    return day;//返回实际的所有病人的住院天数
}
void hospital_money(MedRecord* head)
{
    MedRecord* p;
    p=head;
    int price1=0;
    while(p->next!=NULL){
    for(  int i=0;i<10;i++){
                   price1+=check[(p->medinf.check[i])].price;
            }
            p=p->next;
    }
    p=head;
    ...
    for(int i=0;i<10;i++){
                   ...
                   price2+=drug[(p->medinf.drug[i])].price*(p->medinf.drugqua[i]);
            }
            p=p->next;
    }
    p=head;
    int price3=0;
    price3=in_hospital_day(p)*200;
    int price;
    price=price1+price2+price3;
    printf("医院目前的营业额为：%.3d",price);

}

void patient_statement(MedRecord* head){
     ...
     {for(int i=0;i<10;i++){
      ...
      for(int i=0;i<10;i++){
        ...
        price3=in_hospital_day(p)*200;
        p=p->next;
        printf("检查费用：%.3d    药品费用：%.3d    住院费用：%.3d\n",price1,price2,price3);
     }
